...
**⚠️ 推奨変更**: 実験20251120_09でtest_01014の視覚分析を実施した結果、推奨モデルを変更

#### 視覚分析の結果（実験20251120_09より）
- **特徴量ベースの類似度**: test_01014はwalkingに10.6%近い（距離: walking 3.80 vs stairs 4.24）
- **時系列パターン**: walkingサンプルと視覚的に一致
- **Z軸分析**: 階段昇降の規則的な上下動が見られない
- **結論**: test_01014は**高確率でwalking**

#### 新しい推奨: **ベースラインモデル（20251120_05）**

**推奨理由:**
1. **視覚分析がベースラインを支持**
   - test_01014はwalkingと判定（ベースライン: walking 86.62%、改善版: walking 62.13%）
   - 改善版はstairsに過度に引き寄せられている（stairs 37.24%）
2. **より保守的で安定した予測**
   - 境界サンプルでの確信度が適切
   - stairsクラス重み5倍が過剰調整だった可能性
3. **OOF scoreの限界**
   - 改善版のOOF向上はtrainデータでのstairs改善によるもの
   - testデータでは逆にwalking分類が悪化している可能性

#### 改善版（20251120_07）のリスク
- test_01014のようなwalking-stairs境界サンプルで誤分類の可能性
- Private LBでwalking誤分類によるスコア低下の懸念
- stairsクラス重み調整が過剰

### リスク管理戦略
**両方の予測ファイルを必ず保持**し、Private LB結果で検証：
1. **第一候補**: 20251120_05_baseline_lightgbm（ベースライン）← **変更**
2. **バックアップ**: 20251120_07_stairs_improvement（改善版）
3. Private LB結果で仮説を検証
