"""
LightGBM Stairs Improvement Model for Active Sensor Log Classification
Experiment: 20251120_07_stairs_improvement

Improvements:
- Add stairs-specific features (Z-axis periodicity, Y-Z correlation, Z/Y ratio)
- Add class weight adjustment (stairs weight = 5.0)
"""
...
def extract_features(df, file_id):
    ...
    # ========================================================================
    # Stairs-specific features (Phase 1 improvements)
    # ========================================================================

    # 1. Z-axis periodicity (stairs have characteristic up-down patterns)
    try:
        ...
        # Peak intervals
        if len(peaks_z) > 1:
            peak_intervals = np.diff(peaks_z)
            features['z_peak_interval_mean'] = np.mean(peak_intervals)
            features['z_peak_interval_std'] = np.std(peak_intervals)
        else:
            features['z_peak_interval_mean'] = 0
            features['z_peak_interval_std'] = 0
    except:
        features['z_n_peaks'] = 0
        features['z_n_valleys'] = 0
        features['z_n_cycles'] = 0
        features['z_peak_interval_mean'] = 0
        features['z_peak_interval_std'] = 0

    # 2. Y-Z correlation patterns (different for stairs vs walking)
    # Overall Y-Z correlation (already computed as corr_YZ, but adding more detail)

    # Moving window correlation to capture correlation changes
    window_size = 10
    if len(Y) >= window_size:
        ...
        for i in range(len(Y) - window_size + 1):
            ...
            if np.std(y_window) > 0 and np.std(z_window) > 0:
                corr = np.corrcoef(y_window, z_window)[0, 1]
                rolling_corr_yz.append(corr)

        if len(rolling_corr_yz) > 0:
            ...
            features['yz_rolling_corr_mean'] = 0
            features['yz_rolling_corr_std'] = 0
            features['yz_rolling_corr_max'] = 0
            features['yz_rolling_corr_min'] = 0
    else:
        features['yz_rolling_corr_mean'] = 0
        features['yz_rolling_corr_std'] = 0
        features['yz_rolling_corr_max'] = 0
    ...
    # 3. Z/Y variation ratio (stairs have relatively larger Z variation)
    y_std = np.std(Y)
    z_std = np.std(Z)
    features['z_to_y_std_ratio'] = z_std / (y_std + 1e-8)

    # Also X/Y ratio for completeness
    x_std = np.std(X)
    features['x_to_y_std_ratio'] = x_std / (y_std + 1e-8)

    # Z to magnitude ratio
    features['z_to_mag_std_ratio'] = z_std / (np.std(magnitude) + 1e-8)
...
def create_lgb_datasets(X_train, y_train, X_val, y_val):
    ...
    # Class weights: stairs (label=3) gets 5x weight
